...
db.get('SELECT * FROM vehicle_models WHERE id = 14', (err, model) => {
  ...
  // 查询所有未删除的车型
  db.all('SELECT id, brand, model_name, status FROM vehicle_models WHERE status != \'deleted\' ORDER BY id DESC', (err, models) => {
    if (err) {
      console.error('❌ 查询车型列表失败:', err.message);
    } else {
      console.log(`\n📊 未删除的车型列表 (共 ${models.length} 个):`);
      models.forEach(model => {
        console.log(`   - ID: ${model.id}, 品牌: ${model.brand}, 型号: ${model.model_name}, 状态: ${model.status}`);
      });
      
      const deletedModelStillExists = models.some(model => model.id === 14);
    ...
    // 同时查询所有车型（包括已删除的）
    console.log('\n🔍 查询所有车型（包括已删除的）...');
    db.all('SELECT id, brand, model_name, status, deleted_at FROM vehicle_models ORDER BY id DESC LIMIT 10', (err, allModels) => {
      if (err) {
