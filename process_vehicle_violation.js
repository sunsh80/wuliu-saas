// backend/api/handlers/admin/risk-control/processVehicleViolation.js
const { getDb } = require('../../../../db/index.js');

module.exports = async (c) => {
  console.log("--- 管理员处理车辆违规记录处理器启动 ---");
  ...
  try {
    ...
    const record = await db.get(`
      SELECT vr.*, t.name as target_name, t.avg_rating, o.tracking_number, tv.plate_number
      FROM violation_records vr
      JOIN tenants t ON vr.target_id = t.id
      JOIN orders o ON vr.order_id = o.id
      LEFT JOIN tenant_vehicles tv ON vr.vehicle_id = tv.id
      WHERE vr.id = ?
    `, [recordId]);
    ...
    // 如果是批准处罚，更新车辆的处罚积分
    if ((action === 'approve' || action === 'appeal_reject') && record.vehicle_id && updatedPenaltyPoints > 0) {
      // 更新车辆的处罚积分
      await db.run(`
        ...
        WHERE id = ?
      `, [updatedPenaltyPoints, record.vehicle_id]);

      // 检查处罚积分是否超过阈值，执行相应措施
      const updatedVehicle = await db.get(`
        SELECT id, plate_number, penalty_points 
        FROM tenant_vehicles 
        WHERE id = ?
      `, [record.vehicle_id]);

      if (updatedVehicle.penalty_points >= 100) {
        // 暂停车辆
        await db.run(`
          UPDATE tenant_vehicles 
        ...
        processedNotes += ` | 车辆${updatedVehicle.plate_number}因处罚积分(${updatedVehicle.penalty_points})过高被暂停`;
      } else if (updatedVehicle.penalty_points >= 50) {
        // 发出警告
        processedNotes += ` | 车辆${updatedVehicle.plate_number}因处罚积分(${updatedVehicle.penalty_points})过高被警告`;
      }
    }
    ...
    return {
      ...
      body: {
        ...
        data: {
          ...
          vehicle_info: record.vehicle_id ? {
    ...
    console.error('💥 [PROCESS VEHICLE VIOLATION RECORD ERROR]:', error);
