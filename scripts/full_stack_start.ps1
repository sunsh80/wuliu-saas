...
function StartProjectServices {
    ...
    switch ($mode) {
        "A" {
            ...
            Set-Location -Path "$PSScriptRoot\backend"
            Start-Process -FilePath "cmd" -ArgumentList "/k", "title 物流项目 - Backend API Server && echo 启动Backend API服务... && npm run dev"
        }
        "B" {
            Write-Host "启动Backend API服务..." -ForegroundColor Cyan
            Set-Location -Path "$PSScriptRoot\backend"
            ...
            if (Test-Path "$PSScriptRoot\web") {
                ...
                Start-Process -FilePath "cmd" -ArgumentList "/k", "title 物流项目 - Frontend Web Servers && echo 启动前端Web服务... && npm run dev"
            } else {
