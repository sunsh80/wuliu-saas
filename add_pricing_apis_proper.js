...
if (componentsIndex !== -1) {
  // 在components部分之前插入API端点
  const newContent = content.substring(0, componentsIndex) + pricingEndpoints + content.substring(componentsIndex);
  fs.writeFileSync(openapiPath, newContent);
  console.log('✅ 价格API端点已添加到OpenAPI规范中');
} else {
...
if (!updatedContent.includes('PricingRule')) {
  // 添加schema定义
  const schemaDefinitions = `    PricingRule:
      type: object
      properties:
        id:
          type: integer
...
`;
  ...
  // 查找schemas部分
  const schemasIndex = updatedContent.indexOf('schemas:\n');
  if (schemasIndex !== -1) {
    // 查找responses部分
    const responsesIndex = updatedContent.indexOf('\n  responses:', schemasIndex);
    if (responsesIndex !== -1) {
      // 在responses之前插入schema定义
      const finalContent = updatedContent.substring(0, responsesIndex) + schemaDefinitions + '\n' + updatedContent.substring(responsesIndex);
      fs.writeFileSync(openapiPath, finalContent);
      console.log('✅ 价格相关的Schema定义已添加');
    } else {
      console.log('❌ 未找到responses部分');
    }
  } else {
    console.log('❌ 未找到schemas部分');
  }
}

console.log('✅ 价格API端点和Schema定义添加完成');