// test_data_setup_final.js - 最终修正的测试数据设置脚本
...
async function setupTestData() {
  ...
  try {
    ...
    // 4. 为承运商租户创建用户账号
    console.log('📝 为承运商创建用户账号...');
    const carrierTenants = await db.all("SELECT id, name FROM tenants WHERE name LIKE '%承运%'");
    
    for (let i = 0; i < carrierTenants.length; i++) {
      const tenant = carrierTenants[i];
      const carrierPasswordHash = await bcrypt.hash(`carrier${i+1}234`, 10);
      
      await db.run(
        `INSERT OR IGNORE INTO users (username, email, phone, name, role, type, password_hash, user_type, tenant_id, is_active, status, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'active', datetime('now'), datetime('now'))`,
        [`carrier${i+1}`, `carrier${i+1}@wuliu.com`, `1390013900${i+1}`, `承运商${i+1}`, 'carrier', 'carrier', carrierPasswordHash, 'carrier', tenant.id]
      );
    }
    
    // 5. 为客户租户创建用户账号
    console.log('📝 为客户创建用户账号...');
    const customerTenants = await db.all("SELECT id, name FROM tenants WHERE name LIKE '%客户%'");
    
    for (let i = 0; i < customerTenants.length; i++) {
      const tenant = customerTenants[i];
      const customerPasswordHash = await bcrypt.hash(`customer${i+1}234`, 10);
      
      await db.run(
        `INSERT OR IGNORE INTO users (username, email, phone, name, role, type, password_hash, user_type, tenant_id, is_active, status, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, 'active', datetime('now'), datetime('now'))`,
        [`customer${i+1}`, `customer${i+1}@wuliu.com`, `1380013800${i+5}`, `客户${i+1}`, 'customer', 'customer', customerPasswordHash, 'customer', tenant.id]
      );
    }
    ...
    // 获取承运商用户ID
    const carrierUsers = await db.all("SELECT id FROM users WHERE role = 'carrier'");
    
    if (customers.length >= 2 && carriers.length >= 2 && carrierUsers.length >= 2) {
      for (let i = 1; i <= 6; i++) {
        ...
        const carrierTenantId = i <= 4 ? carriers[0].id : carriers[1].id;
    ...
    console.log('   - 2个客户用户账号');
    console.log('   - 2个承运商用户账号');
