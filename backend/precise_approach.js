...
for (let i = 0; i < lines.length; i++) {
  ...
  if (inSchemasSection) {
    ...
    if (indent === 2 && trimmed && trimmed.startsWith('  ') && /^[A-Z]/.test(trimmed) && trimmed.includes(':')) {
      // 提取 schema 名称（去掉冒号及后面的内容）
      const schemaName = trimmed.substring(2).split(':')[0].trim(); // 去掉前面的2个空格，然后分割取第一部分
      ...
      if (currentIndent === 2 && trimmed && trimmed.startsWith('  ') && /^[A-Z]/.test(trimmed) && trimmed.includes(':')) {
        // 这是下一个 schema 的开始，当前重复 schema 结束
        skippingCurrentSchema = false;
        // 检查这个新 schema 是否也是重复的
        const nextSchemaName = trimmed.substring(2).split(':')[0].trim(); // 去掉前面的2个空格，然后分割取第一部分
