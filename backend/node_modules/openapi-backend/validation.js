...
class OpenAPIValidator {
     ...
     * @memberof OpenAPIValidator
     */
    removeBinaryPropertiesFromRequired(schema) {
        if (typeof schema !== 'object' || !(schema === null || schema === void 0 ? void 0 : schema.required)) {
            return;
        }
        if (schema.properties && schema.required && Array.isArray(schema.required)) {
            ...
            if (binaryProperties.length > 0) {
                schema.required = schema.required.filter((prop) => !binaryProperties.includes(prop));
            }
        }
        ...
        ['allOf', 'anyOf', 'oneOf'].forEach((key) => {
            if (Array.isArray(schema[key])) {
                schema[key].forEach((subSchema) => this.removeBinaryPropertiesFromRequired(subSchema));
            }
        });
    }
    /**
